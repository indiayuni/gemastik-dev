...
rating = document.querySelectorAll('.rating .star');
rating.forEach((item, id)=> {
    item.addEventListener('click', function(){
        for (let index = 0; index < rating.length; index++) {
            if(index <= id){
                rating[index].classList.replace('ri-star-line', 'ri-star-fill');
                rating[index].classList.add('text-yellow-400');
            }
...
>>>>>>> 90ed1d34662285053efacc8a210efe5ca4605218
